...
-- from: (https://stackoverflow.com/questions/38045839/lua-xterm-256-colors-gradient-scripting)
...
local function _strip_hash(rgb)
    -- Strip leading `#` if exists.
    if rgb:find('^#') ~= nil then
        rgb = rgb:sub(2)
    end
    return rgb
end
...
local function rgb2xterm(rgb)
    rgb = _strip_hash(rgb)
    local incs = { 0x00, 0x5f, 0x87, 0xaf, 0xd7, 0xff }
    -- Break 6-char RGB code into 3 integer vals.
    local parts = {}
    for i = 1, 3 do
        table.insert(parts, tonumber(rgb:sub(i*2-1, i*2), 16))
    end
    return nearest_term256_color_index(parts[1], parts[2], parts[3])
end

function M.gui2cterm(gui)
    gui = gui or "none"
    if (gui == "none") then return "none" end
    return rgb2xterm(gui)
end
